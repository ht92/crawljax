...
/**
 * Specifies the actions for CrawlElements NOTE: In general CrawlActions is not
 * designed to be instantiated directly. CrawlActions should be used via
 * {@link CrawlSpecification} To add conditions to check whether a tag should
 * (not) be clicked one can use {@link #when(Condition...)} . See also
 * {@link Condition}
 */
public class CrawlActionsBuilder {
	 ...
	 * <a.../> 2) <div/> click("a") will only include 1 if clickOnce is true
	 * (default). This set can be restricted by {@link #dontClick(String)}.
	 * 
	 * @param tagName
	...
	public CrawlElement click(String tagName) {
		...
		// System.out.println(crawlTag.clickOnce());
		return crawlTag;
	}
	...
	/**
	 * Set of HTML elements Crawljax will click during crawling For exmple 1)
	 * <a.../> 2) <div/> click("a") will only include 1 This set can be
	 * restricted by {@link #dontClick(String)}. If no clicks are specified,
	 ...
	 * clicked. For example: 1) <a href="#">Some text</a> 2) <a class="foo"
	 * .../> 3) <div class="foo" .../> click("a")
